...
// "Migrated from legacy Input Manager to Unity Input System package" || version 1.1.0
...
public class FirstPersonController : MonoBehaviour
{
    ...
    public InputAction moveAction = new InputAction("Move", InputActionType.Value, binding: "<Keyboard>/w",
        interactions: "", processors: "");
    public InputAction lookAction = new InputAction("Look", InputActionType.Value, binding: "<Mouse>/delta",
        interactions: "", processors: "");
    public InputAction jumpAction = new InputAction("Jump", InputActionType.Button, binding: "<Keyboard>/space");
    public InputAction sprintAction = new InputAction("Sprint", InputActionType.Button, binding: "<Keyboard>/leftShift");
    public InputAction crouchAction = new InputAction("Crouch", InputActionType.Button, binding: "<Keyboard>/leftCtrl");
    public InputAction zoomAction = new InputAction("Zoom", InputActionType.Button, binding: "<Mouse>/rightButton");
    ...
    private void Awake()
    {
        ...
        // Setup composite binding for move action (WASD)
    ...
    private void Update()
    {
        ...
        if (cameraCanMove)
        {
            ...
            yaw = transform.localEulerAngles.y + lookDelta.x * mouseSensitivity * 0.1f;

            if (!invertCamera)
            {
                pitch -= mouseSensitivity * lookDelta.y * 0.1f;
            }
            else
            {
                // Inverted Y
        ...
        if (enableSprint)
        {
            ...
            // Handles sprintBar
            if (useSprintBar && !unlimitedSprint && sanitySystem != null)
            {
                float sprintRemainingPercent = sprintRemaining / sprintDuration;
                sanitySystem.SetSprintBar(sprintRemainingPercent);
            }
        }
        ...
        if (playerCanMove)
        {
            ...
            if (enableSprint && sprintAction.IsPressed() && sprintRemaining > 0f && !isSprintCooldown)
            {
                ...
                if (velocityChange.x != 0 || velocityChange.z != 0)
                {
                    ...
                    if (hideBarWhenFull && !unlimitedSprint && sanitySystem != null)
                    {
                        sanitySystem.SetSprintBarAlpha(sanitySystem.GetSprintBarAlpha() + 5 * Time.deltaTime);
                    }
                }
            ...
            else
            {
                ...
                if (hideBarWhenFull && sprintRemaining == sprintDuration && sanitySystem != null)
                {
                    sanitySystem.SetSprintBarAlpha(sanitySystem.GetSprintBarAlpha() - 3 * Time.deltaTime);
                }
...
public class FirstPersonControllerEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        EditorGUILayout.PropertyField(SerFPC.FindProperty("moveAction"));
        EditorGUILayout.PropertyField(SerFPC.FindProperty("lookAction"));
        EditorGUILayout.PropertyField(SerFPC.FindProperty("jumpAction"));
        EditorGUILayout.PropertyField(SerFPC.FindProperty("sprintAction"));
        EditorGUILayout.PropertyField(SerFPC.FindProperty("crouchAction"));
        EditorGUILayout.PropertyField(SerFPC.FindProperty("zoomAction"));
